...
def get_incidents_by_country(
    ...
    country_codes = [
        feature[GeoJsonKeys.PROPERTIES][GeoJsonKeys.ISO_A2_EH]
        for feature in geojson_data[GeoJsonKeys.FEATURES]
    ]

    # Query the dyadic database to get incident counts by country
    grouping_var = (
        DyadicCols.INITIATOR_ALPHA_2
        if incident_type == IncidentType.ATTACKER
        else DyadicCols.RECEIVER_COUNTRY_ALPHA_2_CODE
    )
    query = f"""
    ...
    incidents_by_country = {
        code: 0 for code in country_codes if len(code) == 2
    }  # Initialize with zero counts for all countries
    ...
    for feature in geojson_data[GeoJsonKeys.FEATURES]:
        country_code = feature[GeoJsonKeys.PROPERTIES].get(GeoJsonKeys.ISO_A2_EH)
        feature[GeoJsonKeys.PROPERTIES]["incident_count"] = incidents_by_country.get(
            country_code, 0
        )
        feature[GeoJsonKeys.PROPERTIES]["fill_color"] = get_color_map(
    ...
    gdf = gpd.GeoDataFrame.from_features(geojson_data[GeoJsonKeys.FEATURES])
    return gdf
...
def get_incident_info_by_country(
    ...
    coords_lookup = pd.DataFrame(
        {"lon": centroid.x, "lat": centroid.y}, index=centroid.index
    )
    ...
    result = result.merge(
        coords_lookup,
        left_on=DyadicCols.INITIATOR_ALPHA_2,
        right_index=True,
        how="left",
    ...
    result = result.merge(
        coords_lookup,
        left_on=DyadicCols.RECEIVER_COUNTRY_ALPHA_2_CODE,
        right_index=True,
        how="left",
...
def get_attacker_and_receiver_rankings(
    ...
    unique_incidents_query = f"""
        SELECT DISTINCT {DyadicCols.INCIDENT_ID}, {DyadicCols.INITIATOR_ALPHA_2}, {DyadicCols.RECEIVER_COUNTRY_ALPHA_2_CODE}
        FROM dyadic_source
    """
    ...
    unique_incidents = unique_incidents[
        unique_incidents[DyadicCols.INITIATOR_ALPHA_2].str.len() == 2
    ]
    unique_incidents = unique_incidents[
        unique_incidents[DyadicCols.RECEIVER_COUNTRY_ALPHA_2_CODE].str.len() == 2
    ]

    # Get attacker rankings and rank number based on the number of incidents
    attacker_rankings = (
        unique_incidents.groupby(DyadicCols.INITIATOR_ALPHA_2)
        .size()
        .reset_index(name="incident_count")
        .sort_values(by="incident_count", ascending=False)
    )
    attacker_rankings["rank"] = (
        attacker_rankings["incident_count"]
        .rank(method="min", ascending=False)
        .astype(int)
    )
    # Get receiver rankings
    receiver_rankings = (
        unique_incidents.groupby(DyadicCols.RECEIVER_COUNTRY_ALPHA_2_CODE)
        .size()
        .reset_index(name="incident_count")
        .sort_values(by="incident_count", ascending=False)
    )
    receiver_rankings["rank"] = (
        receiver_rankings["incident_count"]
        .rank(method="min", ascending=False)
        .astype(int)
    )
...
def get_color_map(count: int, max_count: int) -> list:
    ...
    rgba = cmap(normalized_count ** (COLORBAR_SCALE_FACTOR))
